...
$$ \alpha _{0,j} = \alpha _{0} + \mu_{0j} $$

$$ \beta _{1,j} = \beta +1 + \mu_{1j}  $$

which can be re-expressed into a single level by substituting formulae for $\alpha _{0,j}$ and $\beta _{1,j}$ into $y_{i,j}$ as:

$$ y_{i,j} = \beta _0 + \beta_1 x_{ij} + \mu_{0j} + \mu_{1j}x{ij} + \epsilon_{ij} $$

Conceptually, this equation is similar to the null model, but the $\mu _{1j}x_{ij}$ term has been added to reflect the interaction between the $j^{th}$ MSOA and predictor $x$. The random effects $\mu_{0j}$ and $\mu_{1j}$ are assumed: 
...
meaning the slope of regression line for MSOA $j$ is $\beta_1 + \mu_{ij}$.
