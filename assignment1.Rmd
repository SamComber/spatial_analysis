...
# make tweets shapefile from scratch
```
...
# for each folder, get first zon file from each
img <- read.csv("z010_i04.zon")
...
# problem = ids arent matching
spdf <- SpatialPolygonsDataFrame(tes, data = df)

# points in polygon spatial join
pts.poly <- point.in.poly(crimes, spdf)

# aggregate crimes data
pts.agg <- pts.poly@data %>% group_by(id.1) %>% summarise(num = n()) %>% arrange(id.1)

# cast as factor
pts.agg$id.1 <- as.integer(pts.agg$id.1)

# join summed crimes back to data.frame
hex@data <- left_join(hex@data, pts.agg, by = c("ZoneID" = "id.1"))

# test plot
spplot(hex, "num", col="transparent", pretty=TRUE)

cor(hex@data$num, hex@data$id)
...
for (folder in folders) {
  ...
  for (file in files) {
    ...
    # out <- rbind(out, t)
    print(class(t))
    # for each file, we want a list in master list
    out <- rbind(out, file)
...
# each property has an LSOA and an MSOA code, along with IMD info (POINT IN POLYGON)
