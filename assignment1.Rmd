...
pts.agg <- pts.poly@data %>% group_by(ZoneID) %>% summarise(num = n()) %>% arrange(ZoneID)

# join summed crimes back to data.frame
hex@data <- left_join(hex@data, pts.agg, by = c("ZoneID" = "ZoneID"))
