...
To begin examining the intercept and slope residuals, we graphically project the plot of income deprivation slopes by intercept ($\hat{\mu}_{1j}$ by $\hat{\mu}_{0j}$) in \autoref{fig:slope}. A priori, this plot conforms with conventional wisdom: MSOAs with higher-than-average median log housing prices * as shown in the bottom-right quadrant. Interestingly, in the bottom-left quadrant, we observe an MSOA *.
