...
Conceptually, this equation is similar to the null model, but the $\mu _{2j}x_{ij}$ term has been added to reflect the interaction between the $j^{th}$ MSOA and predictor $x$. The random effects $\mu_{0j}$ and $\mu_{2j}$ are assumed: 
...
meaning the slope of regression line for MSOA $j$ is $\beta_2 + \mu_{ij}$, which allows the coefficient to vary by group. We define our random-slopes model in $R$ as follows:
...
To produce the equation for the fitted regression line of MSOA $j$, we estimate: $\hat{y} _{ij} =  (11.36 + \hat{\mu}_{0j}) + (-0.12 + \hat{\mu_{2j}})\textnormal{lr\_income}_{ij}$ where `11.36` and `-0.12` are derived from * and the values of $\hat{\mu}_{0j}$ and $\hat\mu_{2j}$ can be derived from the pairwise residual plot in \autoref{fig:slope}.
...
With our interest in space, we conclude this paper by displaying the spatial distribution of random slopes in \autoref{fig:spplot} using `spplot` for the presentation layer.

```{r spplot, fig.cap = "\\label{fig:spplot}Income deprivation slopes versus intercept."}
setwd("/Users/samcomber/Documents/spatial_analysis/shp/spatial_join")
