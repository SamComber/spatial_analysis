...
To begin interpreting the intercept and slope residuals, we graphically project the plot of income deprivation slopes by intercept ($\hat{\mu}_{2j}$ by $\hat{\mu}_{0j}$) in \autoref{fig:slope}. In all, the plot displays no systematic patterning between the slope and intercept as the number of MSOAs within each quadrant is almost equal. This infers a lack of distinctive between-group patterning for the income deprivation slope and intercept.
...
Next, we compute a caterpillar plot in \autoref{fig:caterpillar} to display the MSOA effects in rank order. From this plot, we observe how the random intercepts does not seem to vary significantly with eachother as indicated by the overlapping 95\% confidence intervals with the exception of one MSOA. Moreover, the effects of income deprivation on median log housing prices does not seem to vary significantly across MSOAs: positive and negative effects are observed, yet they are not significantly different between MSOAs. This infers the absence of between-group significant variance in the effect of income deprivation on median log housing values.
...
Finally, we construct * in \autoref{fig:fan} to illustrate the 'fanning out' of prediction lines.

```{r fan, fig.cap = "\\label{fig:fan}Fanning of prediction lines from random-slope model."}
