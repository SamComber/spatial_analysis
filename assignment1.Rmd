...
$$ y_{i,j} = \alpha _{ij} + \beta _{1}x_{1,ij} + \beta _{2j}x_{2,ij} + \epsilon _{0,ij}$$

$$ \alpha _{0j} = \alpha _{0} + \mu_{0j} $$

$$ \beta _{2j} = \beta +1 + \mu_{1j}  $$

which can be re-expressed into a single level by substituting formulae for $\alpha _{0j}$ and $\beta _{2j}$ into $y_{i,j}$ as:

$$ y_{i,j} = \alpha _0 + \beta_1 x_{ij} + \mu_{0j} + \mu_{2j}x_{ij} + \epsilon_{ij} $$

Conceptually, this equation is similar to the null model, but the $\mu _{1j}x_{ij}$ term has been added to reflect the interaction between the $j^{th}$ MSOA and predictor $x$. The random effects $\mu_{0j}$ and $\mu_{sj}$ are assumed: 

$$\begin{bmatrix} \mu_{0j} \\\ \mu_{1j} \end{bmatrix} \sim \mathcal{N}(0,
\begin{bmatrix}
\sigma_{\mu0}^{2} & \\ 
\sigma_{\mu 0 \mu 1} & \sigma_{\mu1}^{2} 
\end{bmatrix})$$

meaning the slope of regression line for MSOA $j$ is $\beta_2 + \mu_{ij}$, which allows the coefficient to vary by group.
...
Firstly, we use a likelihood ratio test derived from an ANOVA table to estimate whether the income deprivation effect varies across MSOAs. As the deduction of the log-likelihood values computes a likelihood ratio value of 67.47 on 2 degrees of free, we can infer the income deprivation effect varies across MSOAs, justifying the use of a random-slopes model. 

To begin examining the intercept and slope residuals, we graphically project the plot of income deprivation slopes by intercept ($\hat{\mu}_{1j}$ by $\hat{\mu}_{0j}$) in \autoref{fig:slope}. From this, we can derive *.
